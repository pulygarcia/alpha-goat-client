...
export interface DomeImage {
  src: string;
  alt?: string;
  /** Si se pasa, el click en el tile navega acá en vez de agrandar la imagen. */
  href?: string;
}
...
export interface DomeGalleryProps {
  ...
  grayscale?: boolean;
  /** Si es true, la cúpula gira sola en horizontal cuando nadie la arrastra. */
  autoRotate?: boolean;
  /** Velocidad de la auto-rotación en grados/segundo. */
  autoRotateSpeed?: number;
}

/**
 * Galería 3D tipo "cúpula" navegable por drag (React Bits, adaptada a TS).
 * Cada tile clickeable se agranda con una transición FLIP manual sobre el DOM.
 */
  ...
  useEffect(() => {
    ...
    const close = () => {
      ...
      const overlay =
        viewerRef.current?.querySelector<HTMLDivElement>('.enlarge');
      if (!overlay) return;
      const refDiv = parent.querySelector<HTMLDivElement>(
      ...
      if (wantsResize) {
        const onFirstEnd = (ev: TransitionEvent) => {
          ...
          const cleanupSecond = () => {
            overlay.removeEventListener(
              'transitionend',
              cleanupSecond as EventListener,
            );
