...
describe('MarcaIndex', () => {
  it('renders one pill per hoja with its completion percentage', () => {
    ...
    expect(
      screen.getByRole('button', { name: /Águila[\s\S]*100%/ }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole('button', { name: /Havanna[\s\S]*67%/ }),
    ).toBeInTheDocument();
  });
  ...
  it('marks the active pill and calls onSelect with the marca id on click', async () => {
    ...
    expect(screen.getByRole('button', { name: /Havanna/ })).toHaveAttribute(
      'aria-pressed',
      'true',
    );
    expect(screen.getByRole('button', { name: /Águila/ })).toHaveAttribute(
      'aria-pressed',
      'false',
    );
